...
| Dependency vulnerabilities                   | Implemented         | `npm audit` reports 0 vulnerabilities; one prior high finding (`deepmerge-ts` via the Prisma CLI peer dependency) was fixed via a `package.json` override, see `docs/security.md`                   |
...
- Dependency audit: `npm audit --audit-level=critical` (currently 0 vulnerabilities of any severity).
